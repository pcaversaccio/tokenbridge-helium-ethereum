...
Therefore, we are left with deploying a higher-order protocol to implement a decentralised bridge system. Since [Interlay](https://interlay.io) has successfully showcased the [XCLAIM](https://www.xclaim.io) protocol for the `Bitcoin \<\> Polkadot` bridge, we recommend building the `Helium \<\> Ethereum` on the same foundation. The overarching aim is to build an `HNT \<\> ERC20 Wrapped HNT (wHNT)` solution. By deploying `wHNT` on Ethereum, it will be possible to trade the token more easily via automated market makers such as [Uniswap](https://uniswap.org) and thus significantly increase asset liquidity. Assets that have a liquidity premium are up to 25% more valuable, making it even more attractive to become a miner on Helium.
